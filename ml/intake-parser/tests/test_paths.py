...
class TestAnchors:
    ...
    def test_project_dir_is_where_this_projects_pyproject_lives(self):
        """This project's root, not the backend's — the two each own one."""
        assert (paths.PROJECT_DIR / "pyproject.toml").is_file()
        assert paths.PROJECT_DIR != paths.BACKEND_DIR
...
class TestDataSitsBesideThePackageNotInsideIt:
    """The package holds code; datasets and results are its neighbours, not its contents.

    Pinned because the wheel is built from ``packages = ["pipeline"]``: anything that
    drifts back under the package is silently shipped with it, which is how a 968-line
    lab record and every scored run came to live inside an importable module.
    """
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            paths.TRAIN_PATH,
            paths.VAL_PATH,
            paths.PHRASINGS_PATH,
        ],
    )
    def test_is_not_under_the_package(self, path: Path):
        assert paths.PACKAGE_DIR not in path.parents

    @pytest.mark.parametrize("path", [paths.DATASETS_DIR, paths.RESULTS_DIR])
    def test_sits_directly_under_the_project_root(self, path: Path):
        assert path.parent == paths.PROJECT_DIR
...
class TestGeneratedPathsAreAnchoredNotAsserted:
    ...
    def test_lives_in_the_datasets_dir(self, path: Path):
        assert path.parent == paths.DATASETS_DIR
