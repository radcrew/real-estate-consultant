"""Write ``datasets/questions.json`` from the live ``questions`` table.

    cd ml/intake-parser
    python -m pipeline.data.dump_questions
...
training example. Rebuild ``eval.jsonl`` and ``train.jsonl`` after running this, and
