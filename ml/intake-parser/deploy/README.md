# Serving the intake parser model

Two ways to run the same binary with the same flags; the difference is supervision, TLS
and a credential. `llama-server.service`, here, runs it on a host for production.
`pipeline/serve/serve_local.py` runs it on your machine for evaluation — that one is code
and stays in the package, while this directory holds only what is installed on the host.
