...
class SeedManager:
    """Upsert properties and related ``property_images`` via an injected Supabase client."""

    def __init__(self, client: AsyncClient, *, chunk_size: int = 500) -> None:
        self._client = client
        self._chunk_size = chunk_size

    async def delete_property_images(self, property_ids: list[str]) -> None:
        """Remove image rows for the given properties (avoids duplicates on re-seed)."""
        if not property_ids:
            return
        for start in range(0, len(property_ids), self._chunk_size):
            chunk = property_ids[start : start + self._chunk_size]
    ...
    async def insert_properties(self, rows: list[Properties]) -> int:
        if not rows:
        ...
        logger.info(
            "Supabase properties upsert: upserting %d row(s) on conflict (id)",
            len(payload),
        )

        result = await execute_db_safe(
            self._client.table("properties").upsert(payload, on_conflict="id").execute(),
        )
    ...
    async def insert_property_images(self, rows: list[dict[str, Any]]) -> int:
        ...
        for start in range(0, len(rows), self._chunk_size):
            chunk = rows[start : start + self._chunk_size]
            logger.info(
