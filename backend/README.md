...
**Consumer:** `GET /api/v1/listings/{property_id}/similar` — embeds **only the seed**, then runs an indexed k-NN query scoped to the seed's state (widening once if that returns short), returning scores on the same 0–100 scale as search `match_score`. Results are sorted by score, so state decides which rows are eligible, not how they rank. Rows without an embedding are excluded, so run the backfill before relying on it.
