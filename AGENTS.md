...
- `MUST` remember that **opening a pull request deploys a backend preview** (`backend.yml`, the `deploy-preview` job). Opening a PR is an outward-facing action, not just a CI run.
